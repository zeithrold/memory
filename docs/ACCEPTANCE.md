...
- Confirm distinct category suggestions in one turn receive different proposal IDs, while repeated support from one run leaves `evidence_runs` at one.
- Compare catalog tables before and after a dry run: only run, turn, action, metric and scheduling records may change; categories, memberships, proposals and skips must not.
- With the review gate enabled, confirm the first scheduled dry run handles at most six memories, sets `awaitingReview`, and that two later half-hour windows create no instance or token usage. Confirm a manual dry run still starts.
- Return 11 tool calls from a test provider with a budget of 8. Confirm all 11 call ids receive tool results, only 8 execute, 3 are policy rejections, and no provider 400 occurs on the next turn.
- Confirm the Catalog tab shows prompt/completion totals and daily budget progress. At the automatic limit, scheduled dispatch stops while a manual run returns `budgetWarning: true` and continues.
- Confirm an unchanged classification calls `confirm_memberships` and is not selected for seven days; editing the memory must make it eligible immediately. Confirm implicit deferrals wait 6 hours and 24 hours before the final attempt.
- Use a live BYO endpoint that supports `POST /responses`, direct function tools, `tool_choice: "required"`, and `reasoning.effort: "none"`. Confirm the saved service-root URL receives `/responses`, then rerun the connection test because the migration intentionally clears legacy Chat Completions probe results.
- Force `incomplete:max_output_tokens`, a provider-declared failed response, completed-without-tool, invalid JSON/schema, deterministic 4xx, and transient timeout/408/429/5xx failures. Only timeout/408/429/5xx should retry inside the Workflow. Confirm paid usage and the exact terminal reason are journalled, a replay makes no second provider call, the failed batch produces no classifications or implicit skips, and the run is never marked successful/partial. Later scheduled attempts must follow the 2h/6h/24h half-hour-aligned backoff.
- Approve a category proposed for an unclassified batch, then confirm its implicit deferrals are cleared and the same memories can be selected by the next live run.
