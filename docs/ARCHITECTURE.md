...
The workspace uses real routes: `/memories`, `/memories/[id]`, `/catalog`, `/tokens`, `/usage`, and `/connect`; `/` redirects to `/memories`. A shared client shell owns Clerk, locale, observability, toast and auth state but accepts page content as children. Navigation disables prefetch, and each route imports only its own business Client Component so the memories entry path does not fetch management-page chunks.
