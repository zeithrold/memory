...
import { verifyAccessJwt } from './oauth'

/**
 * Credential kinds an entry point accepts. `/mcp` accepts personal tokens and
 * Access JWTs (via Managed OAuth); `/api/v1` normally accepts sessions and
 * personal tokens. The status preflight also accepts Access JWTs under the
 * `oauth` kind, without exposing management operations.
 */
...
export async function authenticate(
  ...
  // Error reports carry the account only as an opaque Access sub: enough to tell
